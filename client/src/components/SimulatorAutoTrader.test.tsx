...
describe("SimulatorAutoTrader disclosure", () => {
  ...
  it("explains that timed automated rounds persist on the server", () => {
    render(
      <SimulatorAutoTrader
        portfolio={{
          id: 1,
    ...
    expect(screen.getByText(/timed 1-day, 3-day, or 7-day run is persisted on the server/i)).toBeTruthy();
    expect(screen.getByText(/background mobile tabs may pause these browser timers/i)).toBeTruthy();
    expect(screen.getByText(/data health: not yet refreshed/i)).toBeTruthy();
    expect(screen.getByText(/last check: not scheduled/i)).toBeTruthy();
  });
