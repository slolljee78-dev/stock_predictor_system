...
import { isStockAlreadyInWatchlist, QUICK_ADD_STOCKS, type QuickAddStock } from "@/pages/dashboard.helpers";
...
export default function Dashboard() {
  ...
  const [quickAddingTicker, setQuickAddingTicker] = useState<string | null>(null);
  const [watchlistFeedback, setWatchlistFeedback] = useState<{ tone: "success" | "error"; message: string } | null>(null);
  const [recentlyAddedTicker, setRecentlyAddedTicker] = useState<string | null>(null);
  ...
  const addToWatchlistMutation = trpc.watchlist.add.useMutation({
    onSuccess: async (_result, input) => {
      ...
      setRecentlyAddedTicker(input.ticker?.toUpperCase() ?? null);
      setWatchlistFeedback({
        tone: "success",
        message: `${input.ticker?.toUpperCase() ?? "Stock"} added to your watchlist.`,
      });
    },
    onError: (error) => {
      console.error('Failed to add stock:', error);
      setAddingStockId(null);
      setWatchlistFeedback({ tone: "error", message: error.message || "Could not add that stock. Please try again." });
    },
  });
  ...
  const recentlyAddedStock = useMemo(
    () => recentlyAddedTicker
      ? watchlist.find((stock) => stock.ticker.toUpperCase() === recentlyAddedTicker)
      : undefined,
    [recentlyAddedTicker, watchlist],
  );
  const recentlyAddedPresentation = useMemo(
    () => recentlyAddedStock
      ? getResolvedWatchlistPresentation(
          recentlyAddedStock.ticker,
          watchlistStatusMap[recentlyAddedStock.ticker],
          latestSignalsByTicker[recentlyAddedStock.ticker],
        )
      : undefined,
    [latestSignalsByTicker, recentlyAddedStock, watchlistStatusMap],
  );
  ...
  const handleAddStock = async (stock: {
    ...
    setAddingStockId(stock.id ?? null);
    await addToWatchlistMutation.mutateAsync({
      stockId: stock.id && stock.id > 0 ? stock.id : undefined,
      ticker: stock.ticker,
      name: stock.name,
  ...
  const handleQuickAddStock = async (stock: QuickAddStock) => {
    setQuickAddingTicker(stock.ticker);
    try {
      await handleAddStock(stock);
    } catch {
      // The mutation's onError handler provides the visible recovery message.
    } finally {
      setQuickAddingTicker(null);
    }
  };
          ...
          <div className="space-y-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div>
                ...
                {watchlistFeedback ? (
                  <p
                    className={`mt-2 text-sm ${watchlistFeedback.tone === "success" ? "text-emerald-300" : "text-rose-300"}`}
                    role={watchlistFeedback.tone === "error" ? "alert" : "status"}
                  >
                ...
                <div className="pt-2 space-y-2">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {QUICK_ADD_STOCKS.map((stock) => {
                      const isInWatchlist = isStockAlreadyInWatchlist(watchlist, { id: -1, ticker: stock.ticker });
                      const isAdding = quickAddingTicker === stock.ticker;
                      return (
                        <button
                          key={stock.ticker}
                          type="button"
                          onClick={() => void handleQuickAddStock(stock)}
                          disabled={isInWatchlist || isAdding}
                          aria-label={isInWatchlist ? `${stock.ticker} is already in your watchlist` : `Add ${stock.ticker} to your watchlist`}
                        ...
                        >
                          {isAdding ? "Adding…" : isInWatchlist ? `${stock.ticker} added` : stock.ticker}
                        </button>
                      );
                    })}
                  </div>
            ...
            {recentlyAddedStock && recentlyAddedPresentation ? (
              <div
                className="rounded-2xl border border-emerald-400/30 bg-[linear-gradient(135deg,rgba(16,185,129,0.14),rgba(14,116,144,0.12))] p-4 shadow-[0_12px_34px_rgba(16,185,129,0.08)]"
                role="status"
                aria-label={`${recentlyAddedStock.ticker} was added to your watchlist`}
              >
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <div className="min-w-0 space-y-1.5">
                    <div className="flex items-center gap-2">
                      ...
                      <p className="font-semibold text-foreground">
                        {recentlyAddedStock.ticker} is now on your watchlist
                      </p>
                    </div>
                    <p className="text-sm text-muted-foreground">
                      {recentlyAddedStock.name} · {recentlyAddedPresentation.statusLabel}
                    </p>
                    <p className="text-sm leading-6 text-muted-foreground">
                      {recentlyAddedPresentation.statusDetail}
                    </p>
                  </div>
                  <div className="flex flex-col gap-2 sm:min-w-44">
                    <Button
                      ...
                      onClick={() => setLocation(hasAutoTradingAccess ? "/simulator#signal-engine" : "/pricing")}
                    >
                      {hasAutoTradingAccess ? "Open Signal Engine" : "Unlock Signal Engine"}
                      <ArrowRight className="h-4 w-4" />
                    </Button>
                    <Button
                      type="button"
                      ...
                      onClick={() => setLocation(buildStockDetailPath(recentlyAddedStock.ticker))}
                    >
                      View {recentlyAddedStock.ticker}
                    </Button>
                  </div>
                </div>
              </div>
