...
const getUserSettingsUseQueryMock = vi.fn();
const updateUserSettingsUseMutationMock = vi.fn();
...
vi.mock("@/lib/trpc", () => ({
  trpc: {
    alertPreferences: {
      getUserSettings: { useQuery: () => getUserSettingsUseQueryMock() },
      updateUserSettings: { useMutation: () => updateUserSettingsUseMutationMock() },
    },
  },
}));
...
describe("AlertPreferencesPage admin test mode", () => {
  ...
  beforeEach(() => {
    ...
    getUserSettingsUseQueryMock.mockReturnValue({ data: undefined, isLoading: false, isFetching: false, refetch: vi.fn() });
    updateUserSettingsUseMutationMock.mockReturnValue({ mutateAsync: vi.fn(), isPending: false });
