...
function calculateBaseSlippage(price: number, spread?: number): number {
  ...
  // Estimate half-spread as a percentage of price. These are deliberately
  // conservative liquid-equity defaults; live liquidity inputs should replace
  // them whenever a venue-specific spread is available.
  if (price >= 100) {
    return 0.00005; // 0.005% for a liquid large-cap quote
  } else if (price >= 50) {
    return 0.0001; // 0.01% for a typical mid-price liquid quote
  } else if (price > 10) {
    return 0.0005; // $0.05 spread for lower-price stocks
  } else {
    return 0.005; // 0.5% baseline spread for low-priced equities
  }
}
...
function calculateOrderSizeImpact(quantity: number, price: number): number {
  ...
  // Use a liquid-equity baseline. A production venue-volume input should
  // replace this assumption for individual instruments.
  const typicalDailyVolume = 20_000_000;
...
function calculateVolatilityImpact(volatility: number = 0.5): number {
  ...
  // Maps to 0% to 0.02% additional slippage in the liquid-equity baseline.
  return volatility * 0.0002;
}
...
function calculateMarketImpact(quantity: number, price: number): number {
  ...
  // Temporary impact grows with participation rate and is capped for this
  // simulator. Live order-book data should supersede this approximation.
  return Math.min(orderSizeRatio * 0.005, 0.01); // Cap at 1%
}
