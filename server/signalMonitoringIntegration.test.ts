...
vi.mock('./hybridMarketData', () => ({
  fetchMultipleMarketData: vi.fn(async (tickers: string[]) => new Map(
    tickers
      .filter((ticker) => ticker !== 'INVALID_TICKER_XYZ')
      .map((ticker) => [ticker, { ticker, currentPrice: 100 }]),
  )),
  fetchMarketDataWithIndicators: vi.fn(),
  checkEventFilter: vi.fn(async () => ({ shouldSuppress: false })),
}));

vi.mock('./realtimeSignalGenerator', () => ({
  generateRealtimeSignal: vi.fn(() => ({
    signalType: 'buy',
    confidence: 70,
    price: 100,
    technicalData: {},
  })),
  validateSignalStrength: vi.fn((signal: { confidence: number }, threshold: number) => signal.confidence >= threshold),
}));

vi.mock('./notificationDelivery', () => ({
  sendBuySignalNotification: vi.fn(async () => undefined),
  sendSellSignalNotification: vi.fn(async () => undefined),
  updateAndNotifySentiment: vi.fn(async () => undefined),
}));
