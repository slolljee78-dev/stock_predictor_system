...
export async function getSignalTrend(days: number = 7) {
  ...
  // Never present generated values as historical activity when data is unavailable.
  if (!db) {
    return {
      buyCount: 0,
      sellCount: 0,
...
export async function getSignalTrendByDay(days: number = 7) {
  ...
  // Preserve the time axis but do not invent historical activity when data is unavailable.
  if (!db) {
  ...
  try {
    ...
    // Preserve the time axis but avoid synthetic performance-like activity on error.
    return Array.from({ length: days }, (_, i) => {
      const date = new Date(Date.now() - (days - i - 1) * 24 * 60 * 60 * 1000);
      return {
        date: date.toISOString().split('T')[0],
...
export async function getSignalAccuracyStats() {
  ...
  // Do not publish synthetic aggregate activity as evidence when DB data is unavailable.
  if (!db) {
    return {
      totalSignals: 0,
      buySignals: 0,
