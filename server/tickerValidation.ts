...
export interface TickerValidationBatchResult {
  validations: TickerValidationResult[];
  supported: TickerValidationResult[];
  unsupported: TickerValidationResult[];
}
 ...
 */
export async function validateMultipleTickers(tickers: string[]): Promise<TickerValidationBatchResult> {
  ...
  return {
    validations: results,
    supported: results.filter((result) => result.isSupported),
    unsupported: results.filter((result) => !result.isSupported),
  };
}
