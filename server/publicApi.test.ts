...
function containsValue(value: unknown, target: string, seen = new Set<object>()): boolean {
  if (value === target) return true;
  if (!value || typeof value !== 'object') return false;
  if (seen.has(value)) return false;
  seen.add(value);
  return Object.values(value).some((entry) => containsValue(entry, target, seen));
}
  ...
  query: {
    stocks: {
      findFirst: vi.fn(async ({ where }: { where: unknown }) => (
        containsValue(where, 'NONEXISTENT') ? undefined : stockFixture
      )),
    },
  },
};
...
describe('Public API', () => {
  ...
  beforeAll(() => {
    ...
    app.use('/api/public', createPublicApiRouter({ getDb: async () => testDb as never }));
  });
  ...
  describe('Error Handling', () => {
    ...
    it('should handle database errors gracefully', async () => {
      ...
      unavailableApp.use('/api/public', createPublicApiRouter({ getDb: async () => null }));
      ...
      expect(res.body).toHaveProperty('error', 'Database unavailable');
    });
  });
});
