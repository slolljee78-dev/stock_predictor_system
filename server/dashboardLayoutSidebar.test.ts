...
describe("DashboardLayout sidebar behavior", () => {
  ...
  it("keeps the sidebar gap and dashboard inset in one shrinkable flex row on desktop", () => {
    const source = fs.readFileSync("client/src/components/DashboardLayout.tsx", "utf8");

    expect(source).toContain('className="dashboard-shell flex min-h-screen w-full"');
    expect(source).toContain('<SidebarInset className="min-w-0 bg-transparent">');
  });
});
