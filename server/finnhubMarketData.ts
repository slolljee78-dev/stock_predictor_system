 ...
 * Replaces Alpha Vantage with better rate limits (60 calls/minute, unlimited daily)
 * Finnhub free tier: 60 API calls per minute, no daily limit
 */

import { ENV } from './_core/env';

const API_KEY = ENV.finnhubApiKey;
const BASE_URL = 'https://finnhub.io/api/v1';
const RATE_LIMIT_DELAY = 1000; // 1 second between calls (60/min = 1 per second)
...
/**
 * Fetch current price data for a stock using Finnhub
 */
export async function fetchCurrentPrice(ticker: string): Promise<PriceData | null> {
  try {
    ...
    if (data.error) {
      console.warn(`[Finnhub] Error fetching price for ${ticker}: ${data.error}`);
      return null;
    }

    if (!data.c) {
      console.warn(`[Finnhub] No price data for ${ticker}`);
      return null;
    }
...
/**
 * Fetch daily OHLCV data for technical analysis using Finnhub
 */
export async function fetchDailyData(ticker: string, limit: number = 100): Promise<PriceData[]> {
  try {
    ...
    const params = new URLSearchParams({
      symbol: ticker,
      resolution: 'D',
      count: Math.min(limit, 500).toString(), // Finnhub returns up to 500 candles
      token: API_KEY,
    });

    const response = await fetch(`${BASE_URL}/stock/candle?${params}`);
    const data = await response.json();

    if (data.error) {
      console.warn(`[Finnhub] Error fetching daily data for ${ticker}: ${data.error}`);
      return [];
    }

    if (!data.o || !Array.isArray(data.o)) {
      console.warn(`[Finnhub] No daily data for ${ticker}`);
      return [];
    }
    ...
    for (let i = 0; i < data.o.length; i++) {
      prices.push({
        timestamp: (data.t[i] || 0) * 1000, // Convert Unix timestamp to milliseconds
        open: data.o[i] || 0,
        high: data.h[i] || 0,
        low: data.l[i] || 0,
        close: data.c[i] || 0,
    ...
    console.error(`[Finnhub] Error fetching daily data for ${ticker}:`, error);
    return [];
  }
}
...
export async function fetchMarketDataWithIndicators(ticker: string): Promise<MarketDataPoint | null> {
  try {
    ...
    console.error(`[Finnhub] Error fetching market data for ${ticker}:`, error);
    return null;
  }
}
